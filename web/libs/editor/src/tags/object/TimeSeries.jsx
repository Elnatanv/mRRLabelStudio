...
const TagAttrs = types.model({
  ...
  cursorcolor: types.optional(
    types.string,
    "var(--color-neutral-inverted-surface)"
  ),
});
...
const Model = types
  ...
  .views((self) => ({
    ...
    get defaultOverviewWidth() {
      ...
      const width =
        Math.min(
          self.overviewwidth.match(/(\d+)%$/)?.[1] ?? defaultWidth,
          100
        ) / 100;
    ...
    get isDate() {
      return (
        Boolean(self.timeformat) ||
        (self.timedisplayformat && /[a-zA-Z]/.test(self.timedisplayformat[0]))
      );
    },
    ...
    get parseTimeFn() {
      return self.timeformat && self.timecolumn
        ? d3.utcParse(self.timeformat)
        : Number;
    },
    ...
    calculateInitialBrushRange(times) {
      ...
      const startIndex = Math.round(
        (times.length - 1) * self.defaultOverviewWidth[0]
      );
      const endIndex = Math.round(
        (times.length - 1) * self.defaultOverviewWidth[1]
      );
      ...
      return self.expandRangeToMinimumPoints(
        times,
        [times[startIndex], times[endIndex]],
        MIN_POINTS_ON_SCREEN
      );
    },
      ...
      if (!self.timecolumn) {
        ...
        for (let i = 0; i < dataLength; i++) {
          ...
          if (self.timeformat) {
            ...
            const parse = actualTimeFormat
              ? d3.utcParse(actualTimeFormat)
              : d3.utcParse(self.timeformat);
          ...
          if (current < previous) {
            const nonSeqValues = [
              `seq: ${i - 1}, value: ${data[self.keyColumn][i - 1]}`,
              `seq: ${i}, value: ${value}`,
            ];
        ...
        const failedValues = timestamps
          .slice(0, 3)
          .filter((t) => t === null || t === 0 || isNaN(t));
        if (failedValues.length >= 2) {
          const message = [
            `<b>timeColumn</b> (${self.timecolumn}) cannot be parsed.`,
            `First wrong values: ${data[self.keyColumn]
              .slice(0, 3)
              .join(", ")}`,
          ];
          ...
          if (self.timeformat) {
            message.push(
              `Your <b>timeFormat</b>: ${self.timeformat}. It should be compatible with these values.`
            );
    ...
    activeStates() {
      ...
      return states
        ? states.filter(
            (s) => s.isSelected && getType(s).name === "TimeSeriesLabelsModel"
          )
        : null;
    },
    ...
    formatTime(time) {
      if (!self._format) {
        ...
        if (format === "date") self._format = formatTrackerTime;
          ...
          self._format = isDate ? d3.utcFormat(format) : d3.format(format);
  ...
  .actions((self) => ({
    ...
    async preloadValue(store) {
      ...
      if (!url || typeof url !== "string") {
        const message = `Cannot find url in <b>${idFromValue(
          self.value
        )}</b> field of your task`;
      ...
      try {
        ...
        const message = `Problems with parsing CSV: ${
          e?.message || e
        }<br>URL: ${url}`;
    ...
    _handleSeek(data) {
      if (!isAlive(self)) {
        console.warn(
          "TimeSeries (seek): model instance is not alive. Skipping operation."
        );
        return;
      }
    ...
    _handlePlay(data) {
      ...
      if (!isAlive(self)) {
        console.warn(
          "TimeSeries (play): model instance is not alive. Skipping operation."
        );
        return;
      }
      if (typeof data.time !== "number" || isNaN(data.time)) {
    ...
    _handlePause(data) {
      ...
      if (!isAlive(self)) {
        console.warn(
          "TimeSeries (pause): model instance is not alive. Skipping operation."
        );
        return;
      }
    ...
    playbackLoop() {
      ...
      const newTimeRelativeSeconds =
        self.playStartPosition + elapsedSeconds * self.playbackSpeed;
    ...
    _updateViewForTime(time) {
      ...
      const [minKey, maxKey] = self.keysRange; // Native units
      if (minKey === undefined || maxKey === undefined || self.canvasWidth <= 0)
        return;
      ...
      const centerPx =
        ((boundedTime - minKey) / timeRangeDurationNative) * self.canvasWidth;
      ...
      const currentPixelWidth =
        (currentBrushWidthNative / timeRangeDurationNative) * self.canvasWidth;
      ...
      let newTimeStartNative =
        minKey + (newPixelStart / self.canvasWidth) * timeRangeDurationNative;
      let newTimeEndNative =
        minKey + (newPixelEnd / self.canvasWidth) * timeRangeDurationNative;
...
const Overview = observer(({ item, data, series }) => {
  ...
  function brushed() {
    if (
      d3.event.selection &&
      !checkD3EventLoop("brush") &&
      !checkD3EventLoop("wheel")
    ) {
      ...
      // if overview was moved; precision comparison for floats
        ...
        prev[0] !== x1 &&
        prev[1] !== x2 &&
        Math.abs(overviewWidth - MIN_OVERVIEW) < 0.001
      ) {
  ...
  React.useEffect(() => {
    ...
    const cursorColor =
      item.cursorcolor || "var(--color-neutral-inverted-surface)";
  ...
  React.useEffect(() => {
    if (item.seekTo !== null && gb.current && isAlive(item)) {
      ...
      const timeRangeDurationNative = maxKey - minKey;
      const centerPx =
        ((seekTimeNative - minKey) / timeRangeDurationNative) * width;
      const range = item.brushRange.map(x);
  ...
  item.regs.map((r) =>
    fixMobxObserve(r.start, r.end, r.selected, r.hidden, r.style?.fillcolor)
  );
...
const HtxTimeSeriesViewRTS = ({ item }) => {
  ...
  const handleMainAreaClick = (event) => {
    if (
      !isAlive(item) ||
      !isFF(FF_TIMESERIES_SYNC) ||
      event.target.closest(".htx-timeseries-overview")
    ) {
      return;
    }
    ...
    if (
      ...
      console.warn(
        "TimeSeries: Click handling skipped, essential data missing or component not ready.",
        {
    ...
    if (plottingAreaWidth <= 0) {
      console.warn(
        `TimeSeries: Plotting area width (${plottingAreaWidth}) is not positive.`
      );
      return;
    }
    ...
    if (brushDurationNative <= 0) {
      console.warn(
        `TimeSeries: Brush duration (${brushDurationNative}) is not positive.`
      );
      return;
    }
    ...
    const timeClicked =
      brushTimeStartNative + (clickX / plottingAreaWidth) * brushDurationNative;
    const [minKey, maxKey] = item.keysRange;
    const finalTime = Math.max(minKey, Math.min(timeClicked, maxKey));
  ...
  return (
    <div
      ...
      onClick={
        ff.isActive(FF_MULTICHANNEL_TS) ? undefined : handleMainAreaClick
      }
    >
      <ObjectTag item={item}>
